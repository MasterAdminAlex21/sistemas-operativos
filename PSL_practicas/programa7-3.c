...
{
   int semid, pid,pid2, j = 10;
   struct sembuf operacion;
   key_t llave;

   //
  // printf("valores de la estructura: %d %d %d\n",operacion.sem_num,operacion.sem_op,operacion.sem_flg);
   //
   ...
   semctl(semid, SEMAFORO_HIJO2, SETVAL, 0);
   ...
   else if (pid == 0)
   {
      ...
      while(j)
      {
       ...
       //  printf("valores de la estructura en el hijo: %d %d %d\n",operacion.sem_num,operacion.sem_op,operacion.sem_flg);
   ...
   else
   {
      ...
      while(j)
      {
         ...
         operacion.sem_op = -1;
         operacion.sem_num = SEMAFORO_PADRE;
       //  printf("valores de la estructura en el padre: %d %d %d\n",operacion.sem_num,operacion.sem_op,operacion.sem_flg);
         semop(semid, &operacion, 1);
